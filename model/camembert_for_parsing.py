...
class Biaffine(nn.Module):
    ...
    def forward(self, x_dep, x_head):
        """
        On ajoute une dimension de biais
        """
...
class CamembertForParsing(nn.Module):
    ...
    def _compute_loss(self, arc_logits, rel_logits, heads, rels, attention_mask):
        """
        On ignore les positions PAD et on gère root=0
        """
        ...
        # on index par heads
        rel_logits_t = rel_logits.permute(0,2,3,1)

        # On doit faire un gather
        heads_ = heads.clone()
        heads_[heads_<0]=0

        batch_idx = torch.arange(bsz).unsqueeze(-1).expand(bsz, seq_len).to(arc_logits.device)
        tok_idx = torch.arange(seq_len).unsqueeze(0).expand(bsz, seq_len).to(arc_logits.device)
        rel_score_for_gold_arc = rel_logits_t[batch_idx, tok_idx, heads_, :]

        rel_score_2d = rel_score_for_gold_arc.view(bsz*seq_len, -1)
        gold_rels = rels.view(-1)

        # on ne calcule la loss rel que sur les positions actives
